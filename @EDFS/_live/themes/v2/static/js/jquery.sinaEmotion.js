function Hashtable() {
    ...
    this.test=function(key){return typeof key;};
}
...
$(function() {
	...
	$.ajax( {
		...
		success : function(response) {
			...
			uSinaEmotionsHt.put('[pt顶一个]', '/themes/v2/static/images/SinaEmotionImages/1.gif');
			uSinaEmotionsHt.put('[pt赞一个]', '/themes/v2/static/images/SinaEmotionImages/2.gif');
			uSinaEmotionsHt.put('[pt掌声]', '/themes/v2/static/images/SinaEmotionImages/3.gif');
			uSinaEmotionsHt.put('[pt鲜花]', '/themes/v2/static/images/SinaEmotionImages/4.gif');
			uSinaEmotionsHt.put('[pt看多]', '/themes/v2/static/images/SinaEmotionImages/5.gif');
			uSinaEmotionsHt.put('[pt看空]', '/themes/v2/static/images/SinaEmotionImages/6.gif');
			uSinaEmotionsHt.put('[pt震荡]', '/themes/v2/static/images/SinaEmotionImages/7.gif');
		}
	});
});
		...
		if (sArr != null)
		{
			for(var i = 0; i < sArr.length; i++){
				if(uSinaEmotionsHt.containsKey(sArr[i])) {
					var reStr = "<img src=\"" + uSinaEmotionsHt.get(sArr[i]) + "\" height=\"auto\" width=\"auto\" />";
...
(function($){
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			var eTop = target.offset().top + target.height() - 350;
	...
	$.fn.SinaEmotion_Mobile = function(target){
		var cat_current;
		var cat_page;
		$(this).click(function(event){
			event.stopPropagation();
			var eTop = $(this).offset().top + $(this).outerHeight() - 320;
			var eLeft = $(this).offset().left - 10;
			...
			$('#emotions').html('<div><a href="javascript:void(0);" id="prev">&laquo;</a><a href="javascript:void(0);" id="next">&raquo;</a></div><div class="categorys"></div><div class="container"></div><div class="page"></div>');
			$('#emotions #prev').click(function(){
				showCategorys(cat_page - 1);
			});
			$('#emotions #next').click(function(){
		...
		function showCategorys(){
			var page = arguments[0]?arguments[0]:0;
			if(page < 0 || page >= categorys.length / 4){
				return;
			}
			$('#emotions .categorys').html('');
			cat_page = page;
		...
		function showEmotions(){
			...
			$('#emotions .page').html('');
			cat_current = category;
			for(var i = page * 66; i < (page + 1) * 66 && i < emotions[category].length; ++i){
				$('#emotions .container').append($('<a href="javascript:void(0);" title="' + emotions[category][i].name + '"><img src="' + emotions[category][i].icon + '" alt="' + emotions[category][i].name + '" width="22" height="22" /></a>'));
			}
			$('#emotions .container a').click(function(){
				target.insertText($(this).attr('title'));
			...
			for(var i = 1; i < emotions[category].length / 66 + 1; ++i){
				$('#emotions .page').append($('<a href="javascript:void(0);"' + (i == page + 1?' class="current"':'') + '>' + i + '</a>'));
			}
			$('#emotions .page a').click(function(){
				showEmotions(category, $(this).text());
