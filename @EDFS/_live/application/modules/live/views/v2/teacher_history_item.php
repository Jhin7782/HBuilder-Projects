...
<div id="audit_<?php echo $v['chatid'] ?>" class="teacher_talk">
			<span>
            <?php if ($v['level'] == 6) { ?>
    ...
    <div class="teacher_talk_name">
        <a href="javascript:void(0)" class="u_mor" ondblclick="javascript:$('#sendMsgInput').val('@'+$(this).html()+': ');"><?php echo $v['chatTeacherName'] ?></a>
        <?php if ($v['toName']) { ?>
            <i>对</i>
            <a href="javascript:void(0)" class="u_mor"
               ondblclick="javascript:$('#sendMsgInput').val('@'+$(this).html()+': ');"><?php echo $v['toName'] ?></a>
            <i>说:</i>
        <?php } ?>
        <a class="teacher_time"><?php echo date("Y-m-d H:i", $v['ctime']) ?></a>
    </div>
    <div class="clear"></div>
